...
*/
/*
let row1 = rows[0].split(/(-|\s|:)/);
console.log(row1);

const [ini,,fin,,letter,,,,pass] = row1
console.log(`ini : '${ini}', fin: '${fin}', letter: '${letter}', pass: '${pass}'`);
*/

console.clear();
const passwordsList= `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
const rows = passwordsList.split("\n")
//console.log(rows)

console.log(` Arreglo rows: ${rows.length} filas: patrones con contraseñas`);
const checkCredentials = (line) =>{
  let row1 = rows[line].split(/(-|\s|:)/);
  //console.log(row1);  
  let [ini,,fin,,letter,,,,pass] = row1
  //console.log(`ini : '${ini}', fin: '${fin}', letter: '${letter}', pass: '${pass}'`);
  let bol=checkPattern(pass,ini,fin,letter)
  //console.log(`valida: ${bol} typeof bol:${typeof bol}`);
  return (bol) ? true :pass
}


const checkPattern = function (password, begin, end, char){    
  //console.log("Función checkPattern");  
  let matches = password.match(new RegExp(char,'g'));
  if(matches===null) return false;
  //console.log(`matches: ${matches.length}`);  
  return (matches.length>=begin && matches.length<=end) ? true: false
}
//console.log(checkPattern("rrrrrrrrrb",ini,fin,letter));

const searchWrongPass = function(num){
  let cont=0;
  let password="";
  rows.forEach((e,ind)=>{
    if(typeof checkCredentials(ind)!=="boolean"){
      password=checkCredentials(ind);
      cont++;
    }
    if(cont===num){
      console.log(`Invalid password Nro: ${num} --> '${password}'`);
      return password
    }
  });
}               
